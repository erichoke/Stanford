...
function ExtractPL_Interp(xvals,subtractmin)
xvals=xvals';
...
load('SpectrometerResponse','SpectrometerResponse')
...
headernames2='Wavelength (nm), Energy (eV),';
...
for j=1:length(PLfiles)
    ...
    calib = interp1(SpectrometerResponse(:,1),SpectrometerResponse(:,2),xvals); % Correct for detector response
    calib_sphere = interp1(SphereSpectrometerResponse(:,1),SphereSpectrometerResponse(:,2),xvals); % Correct for detector response
    ...
    Spheredata=[PLdata, yvals./calib/IntegerationTime];
    PLdataNorm=[PLdataNorm, yvals./calib_sphere/max(yvals./calib)];
    ...
    temp=PLfiles(j).name;
    headernames=strcat(headernames,temp(1:end-9), ','); % Names with extensions removed
    headernames2=strcat(headernames2,temp(1:end-9), ','); % Names with extensions removed    
end

% write file
dlmwrite(strcat(pathname,'PLRaw.csv'),headernames, '');
dlmwrite(strcat(pathname,'PLRaw.csv'),PLdataRaw,'-append');
dlmwrite(strcat(pathname,'PLData.csv'),headernames, '');
dlmwrite(strcat(pathname,'PLData.csv'),PLdata,'-append');
dlmwrite(strcat(pathname,'PLSphere.csv'),headernames, '');
dlmwrite(strcat(pathname,'PLSphere.csv'),Spheredata,'-append');
dlmwrite(strcat(pathname,'PLDataNorm.csv'),headernames, '');
dlmwrite(strcat(pathname,'PLDataNorm.csv'),PLdataNorm,'-append');
dlmwrite(strcat(pathname,'PLDataEnergy.csv'),headernames2, '');
dlmwrite(strcat(pathname,'PLDataEnergy.csv'),PLdataeV,'-append');

%xlswrite(filepath, headernames, 'RawCountsPerSec', 'A1')
%xlswrite(filepath, RawCountsPerSec, 'RawCountsPerSec', 'A2')
% xlswrite(filepath, headernames, 'Response Corrected', 'A1')
% xlswrite(filepath, PLdata, 'Response Corrected', 'A2') 
